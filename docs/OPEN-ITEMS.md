...
| **Ansible student on/off-boarding playbook** | `OPEN` (blocked on roster) | Kyle 2026-07-15: build a `student_access` role in `Home-Lab/playbooks` once real student IDs arrive. Design: semester roster var file (`studentNN` -> name/email/pubkey); **onboard** = load keys (authorized_key module or wrap `add-cluster-key.sh`, which validates + dedups); **offboard** = clear `authorized_keys`, optionally archive-then-wipe `/workspace/students/<u>` + `/workspace/scratch/<u>` (homes are PBS-backed nightly, so archive may be redundant); later add Cloudflare Access email add/remove via API once Zero-Trust is signed up (seats free on user removal). Runs from Ares control node like the hardening role. Generic accounts make rotation = key swap + home wipe. |
