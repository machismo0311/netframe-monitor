...
| **Revive OPNsense guest agent (VM 100)** | `OPEN` | Went unresponsive 2026-07-17 (`qm agent 100 ping`=not running, though enabled). OPNsense healthy otherwise. Needed for: graceful shutdown on pve2 reboot (config-corruption risk if down), the cache-proof DR backup (backup.sh reads config via it), and the VLAN20 clamp apply. Revive: serial console `qm terminal 100` from pve2 -> `service qemu-guest-agent restart` (or GUI). Has been finicky here historically. |
